...
import { saveGameResult } from '../services/cloudStore';
...
export interface Room {
  ...
  /** Kết quả đáp án câu hiện tại */
  answerResult?: { correctAnswer: number; explanation: string; answers: Record<string, number> } | null;
  /** Team assignments: playerId → teamId */
...
interface GameContextType {
  ...
  assignTeams: (assignments: Record<string, string>, teams: TeamInfo[]) => Promise<void>;
  players: Player[];
  currentQuestion: any;
...
export function GameProvider({ children }: { children: React.ReactNode }) {
  ...
  // Accumulate all answers per question for game history
  const allAnswersHistoryRef = useRef<Record<number, Record<string, number>>>({});
  ...
  const nextQuestion = useCallback(async () => {
    ...
    if (nextIdx < room.questions.length) {
      ...
      setGameState('finished');
      // Save game result to cloud + localStorage
      try {
        const freshSnap = await get(ref(database, `rooms/${pin}`));
        if (freshSnap.exists()) {
          const finalRoom = freshSnap.val() as Room;
          const playersArr = Object.values(finalRoom.players || {});
          const sorted = [...playersArr].sort((a, b) => b.score - a.score);
          const questionResults = (finalRoom.questions || []).map((q, qi) => ({
            index: qi,
            content: q.content,
            correctAnswer: q.correctAnswer,
            playerAnswers: allAnswersHistoryRef.current[qi] ?? {},
          }));
          await saveGameResult({
            pin,
            title: finalRoom.questions?.[0]?.subject || `Phòng ${pin}`,
            date: Date.now(),
            durationMs: Date.now() - gameStartTimeRef.current,
            totalQuestions: finalRoom.questions.length,
            players: playersArr.map(p => ({
              id: p.id, name: p.name, avatar: p.avatar,
              score: p.score, correctAnswers: p.correctAnswers, isEliminated: p.isEliminated,
            })),
            questionResults,
            topPlayerName: sorted[0]?.name ?? '',
            topPlayerScore: sorted[0]?.score ?? 0,
          });
        }
      } catch (err) {
        console.warn('[GameContext] Không lưu được kết quả game:', err);
      }
    }
  }, [room]);
  ...
  const showAnswer = useCallback(async () => {
    ...
    // Track answers history for this question
    const answersMap: Record<string, number> = {};
    Object.values(updatedPlayers).forEach(p => {
      const ans = roomAnswers[p.tabId];
      if (ans !== undefined) answersMap[p.id] = ans;
    });
    allAnswersHistoryRef.current[freshRoom.currentQuestionIndex] = answersMap;
  ...
  const assignTeams = useCallback(async (assignments: Record<string, string>, teams: TeamInfo[]) => {
    if (!room) return;
    const pin = room.pin;
    const teamsObj: Record<string, TeamInfo> = {};
    teams.forEach(t => { teamsObj[t.id] = t; });
    await update(ref(database), {
      [`rooms/${pin}/teamAssignments`]: assignments,
      [`rooms/${pin}/teams`]: teamsObj,
    });
  }, [room]);
  ...
  return (
    <GameContext.Provider value={{
      ...
      joinRoom, submitAnswer, assignTeams,
