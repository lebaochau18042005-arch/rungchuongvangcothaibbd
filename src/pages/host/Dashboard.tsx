...
import { Plus, Play, Sparkles, BookOpen, Settings, Trash2, Pencil, X, Tag, Upload, FileText, Loader2, Download, History } from 'lucide-react';
...
import { exportQuestionsToDocx } from '../../services/exportDocx';
import { exportQuestionsToPptx } from '../../services/exportPptx';
...
export default function HostDashboard() {
  ...
  const { apiKey, selectedModel, requestApiKey } = useApiKey();
  ...
  const generateWithAI = async () => {
    ...
    try {
      const generated = await generateQuestions(key, topic, count, difficulty, selectedModel);
      ...
      <div className="max-w-6xl mx-auto space-y-8">
        <header className="flex justify-between items-center flex-wrap gap-4">
          ...
          <div className="flex items-center gap-3">
            <button onClick={() => navigate('/host/history')}
              className="px-4 py-3 bg-slate-700 text-white rounded-xl font-bold shadow-md flex items-center gap-2 hover:bg-slate-800 transition">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="space-y-6">
            ...
            <div className="bg-white p-6 rounded-2xl shadow-sm border border-slate-200 space-y-4">
              ...
              {/* Team Mode */}
              <label className="flex items-center gap-3 p-3 border border-slate-200 rounded-xl cursor-pointer hover:bg-slate-50">
                <input type="checkbox" checked={settings.teamMode}
                  onChange={e => setSettings({...settings, teamMode: e.target.checked})}
                  className="w-5 h-5 text-orange-500 rounded focus:ring-orange-500" />
                <span className="font-medium text-slate-700">🏆 Chế độ đội (Team Battle)</span>
              </label>
              {settings.teamMode && (
                <div>
                  <label className="block text-sm font-medium text-slate-600 mb-1">Số đội</label>
                  <select value={settings.teamCount}
            ...
            <div className="bg-white p-6 rounded-2xl shadow-sm border border-slate-200 space-y-4">
              <h2 className="text-xl font-bold flex items-center gap-2 text-slate-800">
                <Download size={20} /> Xuất câu hỏi
              </h2>
              <p className="text-sm text-slate-500">Xuất {filteredQuestions.length} câu hỏi {filterSubject ? `(môn ${filterSubject})` : '(tất cả)'} ra file.</p>
              <div className="grid grid-cols-2 gap-3">
                <button
                  disabled={filteredQuestions.length === 0 || isExporting}
                  onClick={async () => {
                    setIsExporting(true);
                    await exportQuestionsToDocx(filteredQuestions, filterSubject || 'Ngân hàng câu hỏi');
                    setIsExporting(false);
                  }}
                  className="py-2.5 bg-blue-600 hover:bg-blue-700 text-white rounded-xl font-bold text-sm flex items-center justify-center gap-2 disabled:opacity-50 transition">
                ...
                <button
                  ...
                  onClick={async () => {
                    setIsExporting(true);
                    await exportQuestionsToPptx(filteredQuestions, filterSubject || 'Ngân hàng câu hỏi');
                    setIsExporting(false);
                  }}
                  className="py-2.5 bg-orange-600 hover:bg-orange-700 text-white rounded-xl font-bold text-sm flex items-center justify-center gap-2 disabled:opacity-50 transition">
